...
export async function POST(
  ...
  if (intent === "delete") {
    ...
    if (error) return redirectToEvents(req, "error", `Could not delete event: ${error.message}`);
    return redirectToEvents(req, "success", "Event deleted.");
  }
  ...
  const imageUrl = uploadedImageUrl ?? (removeImage ? null : fieldImageUrl);
  ...
  if (error) return redirectToEvents(req, "error", `Could not save event: ${error.message}`);

  return redirectToEvents(req, "success", "Event changes saved.");
}
